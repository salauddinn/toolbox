  ...
  switch (input.stage.kind) {
    ...
    case "cycle_repair": {
      ...
      return [
        ...
        `Every target file (the module public index.js, ${input.analysis.entryPath}, and the evidenced cycle files) ALREADY EXISTS from earlier stages. Emit an "update" operation for each and never "create" — creating an existing path fails to apply.`,
        "The module public index.js already wires the domain's routes. Keep that route wiring: place it inside the factory and return the router alongside the injected dependency, so every route listed above stays registered.",
        "Break the cycle in the evidenced cycle files by removing the selected domain's require of the other cycle file; route that dependency through the factory instead.",
